...
module.exports = function(_options) {
	...
	for(var i = 0, len = pkgs.length;i < len;i ++) {
      ...
      var pkgInfo = false
      try {
        pkgInfo = require( rpath + '/node_modules/' + fo + '/package.json' )
      } catch (e) {
        console.log(fo, 'no package.json')
      }
	...
	for(var j = 0, len0 = varr.length;j < len0;j ++) {
		...
		var rep = prefix === '*'?'*':prefix + obj.version

		if(!reg.test(pkgStr)) console.log(obj.name + ' is not in the package.json')
		else pkgStr = pkgStr.replace(reg, '"' + obj.name + '": "' + rep + '"')
